...
Step 6 of ``docs/TAXONOMY_IMPLEMENTATION_PLAN.md``, and the step the plan schedules alone. Three
builders currently write the taxonomy by locating their own block in the byte stream and
...
is source knowledge and stays exactly where it is. What is replaced is the ~120 lines at the end of
the three that actually write.

Four properties, each answering a specific way the old write path went wrong:
...
class _Minter:
    ...
    def resolve(self, lineage, concept) -> str:
        ...
        for depth in range(1, len(lineage) + 1):
            ...
            existing = self.by_key.get(key)
            parent = existing["taxonID"] if existing else self.mint(key, parent, rank, name, "taxon", "")
        return parent
...
def _add_ids(id_rows, dataset, verbatim, subject, ids) -> None:
    ...
    for legacy_column, value in (ids or {}).items():
        ...
        if legacy_column not in MULTI_VALUED_COLUMNS:
            ...
            held = [key[1] for key in id_rows if key[0] == subject and key[1].startswith(f"{prefix}:")]
            if held and held != [f"{prefix}:{values[0]}"]:
                raise TaxonomyError(
                    f"{dataset}/{verbatim}: {subject} already holds {held[0]} for {legacy_column}; the record says "
...
def upsert_source(package_dir, dataset: str, records, *, apply: bool = False, allow_delete: bool = False) -> ChangeSet:
    ...
    for record in records:
        ...
        _add_ids(id_rows, dataset, verbatim, taxon_id, record.get("ids"))
