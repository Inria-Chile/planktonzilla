...
│   │   ╰── utils/                            # extract_cox.py, extract_taxon_ids.py, verify_taxon_ids.py, KNOWN_ISSUES.md, RESOLVED_ISSUES.md
...
```bash
# Create the whole dataset from scratch (all 20 sources) — the default
uv run pz_planktonzilla
...
Fifteen public plankton-imaging sources are assembled into `planktonzilla-17M`. Five more are in
the build registry since v1.2 and enter the composite with the v1.2 release: **FREPJ-Z** (published
on its own meanwhile as
[`project-oceania/planktonzilla-frepj`](https://huggingface.co/datasets/project-oceania/planktonzilla-frepj))
and the four **Tara Pacific** deposits of [Mériguet et al.
...
sources plus the five joining in v1.2, so a from-scratch rebuild reproduces the same mix of terms —
`frepj` and the four `tara_pacific_*` sources add `cc-by-4.0` images only.
